...
export type TokenType =
  | "IF" | "THEN" | "ALERT" | "AND" | "OR"
  | "IDENT" | "NUMBER" | "OP" | "LEVEL"
  | "LPAREN" | "RPAREN" | "EOF";
...
const KEYWORDS: Record<string, TokenType> = {
  IF: "IF", THEN: "THEN", ALERT: "ALERT", AND: "AND", OR: "OR",
};
...
export function tokenize(src: string): Token[] {
  ...
  while (i < src.length) {
    ...
    if (OPS.includes(op2)) { tokens.push({ type: "OP", value: op2, pos: i }); i += 2; continue; }
