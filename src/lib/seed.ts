...
const DEFAULT_RULES = [
  { name: "Tachycardia + Hypoxia", source: "IF heart_rate > 110 AND spo2 < 93 THEN ALERT CRITICAL", severity: "CRITICAL" },
  ...
  if (!existing || existing.length === 0) {
    for (const p of DEFAULT_PATIENTS) {
      ...
      await supabase.from("patients").insert({
        ...
        display_label: p.ext,
        baseline_hr: p.hr, baseline_spo2: p.spo2, baseline_temp: p.temp,
      });
    }
  }
    ...
    for (const r of DEFAULT_RULES) {
      try {
        ...
        await supabase.from("rules").insert({
          ...
          enabled: true, severity_default: r.severity,
        });
      } catch { /* ignore */ }
    }
  }
