...
function PatientDetail() {
  ...
  const [alerts, setAlerts] = useState<
    { id: string; level: string; message: string; ts: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data: v } = await supabase
        .from("vitals")
        .select("ts, smoothed_hr, smoothed_spo2, smoothed_temp, is_anomaly")
        .eq("patient_id", id)
  ...
  const chartData = vitals.map((v) => ({
    t: new Date(v.ts).toLocaleTimeString([], {
      hour: "2-digit",
      minute: "2-digit",
      second: "2-digit",
    }),
      ...
      <header className="mb-6">
        ...
        {patient && (
          <p className="text-xs text-muted-foreground">
            Baselines: HR {patient.baseline_hr} · SpO₂ {patient.baseline_spo2} · Temp{" "}
            {patient.baseline_temp}°C
          </p>
        )}
      </header>
            ...
            <ul className="divide-y">
              {alerts.map((a) => (
                <li key={a.id} className="py-2 flex items-center gap-3">
                  <Badge variant={(a.level === "CRITICAL" ? "destructive" : "secondary") as never}>
                    {a.level}
                  </Badge>
...
function ChartCard({
  ...
  dataKey: string;
  data: { t: string; [k: string]: string | number }[];
  color: string;
}) {
  return (
    <Card>
